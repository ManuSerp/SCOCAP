...
RGBColor::RGBColor(uint8_t _r, uint8_t _g, uint8_t _b) { SetColor(_r, _g, _b); }
...
class Image {
   public:
    ...
    Image() {
        width = height = 0;
    };  // Don't use default constructor, just for variable definition
    ...
    Image(int64_t _width, int64_t _height, const string& _outFileName,
          const RGBColor& _backgroundColor);
    void SetPixel(int64_t x, int64_t y, const RGBColor& color, bool ignore_err);
    const RGBColor& GetPixel(int64_t x, int64_t y) const;
    void DrawLine(int64_t x0, int64_t y0, int64_t x1, int64_t y1,
                  const RGBColor& color);
    void DrawCircle(int64_t x0, int64_t y0, int64_t r, const RGBColor& color,
                    bool fill);
   ...
   private:
    ...
    void Setup();
    void FillBG();
    void DrawLineLow(int64_t x0, int64_t y0, int64_t x1, int64_t y1,
                     const RGBColor& color);
    void DrawLineHigh(int64_t x0, int64_t y0, int64_t x1, int64_t y1,
                      const RGBColor& color);
...
Image::Image(const string& _inFileName) { OpenSetup(_inFileName); }
...
void Image::Init(int64_t _width, int64_t _height) {
    if (_width <= 0 || _height <= 0)
        throw std::invalid_argument(
            "EasyBMP ERROR: Image width and heigth must be greater than zero");
    width = _width;
    height = _height;
    buffer = NULL;
...
void Image::Setup() {
    try {
        ...
        std::stringstream ss;
        ss << "EasyBMP ERROR: bad_alloc error (Can't create image buffer) -> "
           << ba.what();
    ...
    if (!isValidCoordinate(x, y)) {
        if (!ignore_err)
            throw std::out_of_range(
                "EasyBMP ERROR: pixel coordinate is out of range.");
    } else {
        buffer[y][x] = color;
    }
}
...
void Image::DrawLine(int64_t x0, int64_t y0, int64_t x1, int64_t y1,
                     const RGBColor& color) {
...
void Image::DrawLineLow(int64_t x0, int64_t y0, int64_t x1, int64_t y1,
                        const RGBColor& color) {
    int64_t dx = x1 - x0;
    int64_t dy = y1 - y0;
    int64_t yi = 1;
...
void Image::DrawLineHigh(int64_t x0, int64_t y0, int64_t x1, int64_t y1,
                         const RGBColor& color) {
    int64_t dx = x1 - x0;
    int64_t dy = y1 - y0;
    int64_t xi = 1;
...
void Image::DrawCircle(int64_t x0, int64_t y0, int64_t r, const RGBColor& color,
                       bool fill = false) {
    if (x0 < 0 and y0 < 0 and x0 >= width and y0 >= height)
        throw std::out_of_range(
            "EasyBMP ERROR: circle center coordinate is out of range.");
...
void Image::SetFileName(const string& _outFileName) {
    if (_outFileName.size() == 0)
        throw std::out_of_range(
            "EasyBMP ERROR: File name must contain at least one character.");
...
void Image::Write() {
    ...
    if (not outFile.is_open()) {
        throw std::runtime_error(
            "EasyBMP ERROR: Can't open file to write data.");
    }
...
void Image::OpenSetup(const string& _inFileName) {
    ...
    if (!inFile.is_open()) {
        throw std::runtime_error(
            "EasyBMP ERROR: Can't open file to read data.");
    }
    ...
    if (compression != 0) {
        throw std::runtime_error(
            "EasyBMP ERROR: Bitmap compression not supported.");
    }

    if (bytesPerPixel != 3) {
        throw std::runtime_error(
            "EasyBMP ERROR: Bytes per pixel of bitmap must be 3.");
    }
